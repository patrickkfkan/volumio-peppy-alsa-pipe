...
class PeppyAlsaPipeContext {
  ...
  toast(type: 'success' | 'info' | 'error' | 'warning', message: string, title = 'Peppy ALSA Pipe') {
    this.#pluginContext.coreCommand.pushToastMessage(type, title, message);
  }

  refreshUIConfig() {
    this.#pluginContext.coreCommand.getUIConfigOnPlugin('audio_interface', 'peppy_alsa_pipe', {}).then((config: any) => {
  ...
  getAlsaController(): any {
    return this.#getSingleton('alsaController', () => this.#pluginContext.coreCommand.pluginManager.getPlugin('audio_interface', 'alsa_controller'));
  }
...
export default new PeppyAlsaPipeContext();
